...
<style>
  ...
  .lp .rosette{position:absolute;top:2px;left:2px;width:156px;height:156px;animation:lpspin 9s linear infinite;opacity:.4}
...
export default function Landing({ onEnter }) {
  ...
  useEffect(() => {
    ...
    // ambient audio (generated)
    let master = null, soundOn = true;
    function buildAudio() {
      actx = new (window.AudioContext || window.webkitAudioContext)();
      master = actx.createGain(); master.gain.value = 0; master.connect(actx.destination);
      [110, 110 * 1.5, 220].forEach((f, i) => { const o = actx.createOscillator(), g = actx.createGain(); o.type = "sine"; o.frequency.value = f; o.detune.value = (i - 1) * 6; g.gain.value = [.10, .05, .04][i]; const lfo = actx.createOscillator(), lg = actx.createGain(); lfo.frequency.value = .05 + i * .03; lg.gain.value = g.gain.value * .6; lfo.connect(lg); lg.connect(g.gain); lfo.start(); o.connect(g); g.connect(master); o.start(); });
    }
    function startAudio() { if (!soundOn) return; if (!actx) buildAudio(); if (actx.state === "suspended") actx.resume(); ramp(.22); }
