    ...
    steps: [
      {
        ...
        detail: (ctx) => {
          if (ctx.monthsToFund === null) {
            return `Projected take-home in ${ctx.cityName}: $${ctx.monthlyTakeHome.toLocaleString()}/month. Current expense estimate leaves a deficit, so expenses exceed income at this location. Review each expense category and identify reductions before committing to a move date.`;
          }
    ...
    steps: [
      {
        ...
        detail: (ctx) => {
          if (ctx.monthsToFund === null) {
            return `Projected take-home in ${ctx.cityName}: $${ctx.monthlyTakeHome.toLocaleString()}/month (USD equivalent). Current expense estimate leaves a deficit, so expenses exceed income at this location. Review each expense category and identify reductions before committing to an international move date.`;
          }
      ...
      },
      {
        ...
        detail: (ctx) => {
          ...
          return `Target international relocation fund: $${ctx.targetFundUSD.toLocaleString()}, covering UK visa application fees, international shipping or storage, first + last month deposit in ${ctx.cityName}, transatlantic flights, and a 3-month emergency reserve. At $${ctx.monthlySavings.toLocaleString()}/month saved, you reach this in ${ctx.monthsToFund} month${ctx.monthsToFund === 1 ? '' : 's'}. Currency risk is real, so consider converting a portion to GBP when your fund is near target.`;
        },
      },
    ],
  },
    ...
    steps: [
      {
        ...
        detail: (ctx) => {
          ...
          return `Median home price in ${ctx.cityName}: $${ctx.medianHome.toLocaleString()} (USD equivalent). London's market is competitive; a 10-20% deposit is standard. As a US buyer without UK credit history, you may face higher mortgage rates initially, so some buyers rent for 12-24 months to build UK credit before purchasing. Factor in Stamp Duty Land Tax (SDLT) and legal fees (~2-4% of purchase price).`;
        },
      },
      {
        label: 'Neighborhood research',
